...
export default function PostsPage() {
  ...
  useEffect(() => {
    const firstSelected = accounts.find((item) =>
      selectedAccountIds.includes(item.id)
    );

    if (firstSelected?.platform) {
      setSelectedPlatform(firstSelected.platform.toLowerCase());
    }
  ...
  async function fetchAccounts() {
    try {
      ...
      // ✅ Auto select first account only one time
      if (accountList.length > 0) {
        setSelectedAccountIds((prev) =>
          prev.length > 0 ? prev : [accountList[0].id]
        );

        if (accountList[0]?.platform) {
          setSelectedPlatform(accountList[0].platform.toLowerCase());
        }
      }
    } catch (error) {
  ...
  async function handlePublishNow() {
    ...
    try {
      ...
      // ✅ MULTI ACCOUNT API
      const res = await fetch(`${API_BASE_URL}/api/multi-posts`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error(data?.message || "Failed to create multi-account post");
      }
      ...
      <div className="px-6 py-6">
        <div className="mb-6 rounded-[28px] border border-white/10 bg-[#030d1d] p-6 shadow-[0_0_40px_rgba(59,130,246,0.08)]">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Create one post and publish it to multiple selected social accounts.
          </p>
        </div>
